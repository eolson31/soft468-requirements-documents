...
- Given the tester is at the dashboard
  When the tester clicks "Begin Test"
  Then a new hearing test session is created

- Given the tester is in a test session
  When the test type is selected
  Then the system initializes left and right ear testing sequences

- Given the tester is in a test session
  When a hearing test has been completed
  Then the test results are uniquely identified and saved

### FR2 – Multi-Frequency Tone Generation (G1)

- Given a diagnostic hearing test is running
  When tones are generated
  Then tones at 500, 1000, 2000, 4000, and 8000 Hz are played

- Given a hearing screening is running
  When tones are generated
  Then tones at 1000, 2000, 4000, and 6000 Hz are played
...
- Given an administrator creates a user account with a valid email
...
- Given a user has been approved
  ...
  Then access to upload/download features is enabled
...
- Given data synchronization is complete
  ...
  Then locally stored data is removed
...
- Given an audiogram is displayed
  When it is rendered
  Then it includes plotted values for all relevant frequencies
